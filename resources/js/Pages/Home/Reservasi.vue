<script setup>
import { ref, watch, defineProps, computed, reactive } from 'vue';
...
// Cek Param
function dataURL() {
    var page = usePage().url;
    if (page.indexOf('?') === -1) {
        return {}
    }
    page = page.substring(page.indexOf('?') + 1);
    return Object.assign(Object.fromEntries(new URLSearchParams(page)))
}
const objLength = Object.keys(dataURL()).length
...
<template>
    <div
        class="container mx-auto flex flex-col sm:flex-col md:flex-row md:items-center px-2 py-3 sm:py-5 md:py-10 bg-primary rounded-2xl">
        <loadingAnimation :show="loadingPage" />
        <h1 class="font-bold md:w-1/5 xl:w-1/4 text-white leading-6 tracking-wide">Isi Form Reservasi Kamar Wisma Malaqbi
        </h1>
        ...
        <form class="flex flex-col sm:flex-col lg:flex-row md:space-x-4 py-2 px-4 w-4/5" @submit.prevent="submit">
            ...
            <div class="block">
                ...
                <div class="flex flex-row">
                    <PrimaryButton type="button" @click="minusGuest"
                        class="bg-transparent hover:bg-transparent focus:bg-transparent active:bg-transparent !border-none !ring-0 focus:!ring-0 focus:!border-none active:!ring-0 focus:outline-none outline-none active:border-none !px-3">
            ...
            <div class="flex border-t border-white pt-3 md:border-none">
                <PrimaryButton
                    class="bg-white !text-primary hover:text-red-600 font-bold hover:!bg-gray-200 active:bg-gray-100 focus:bg-gray-200">
                    Cari</PrimaryButton>
            </div>
        </form>
    </div>
